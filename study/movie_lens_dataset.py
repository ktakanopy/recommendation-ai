...
class MovieLensTrainDataset(Dataset):
    ...
    def __init__(
        self, ratings, precomputed_candidates, feature_processor, num_negatives=4
    ):
    ...
    def get_dataset(self):
        ...
        print("Generating training dataset with precomputed candidates")
        ...
        for user_id, positive_items in tqdm(
            user_positive_items.items(), desc="Processing users"
        ):
        ...
        print(
            f"Generated {len(user_features_list)} samples ({labels.count(1)} positive, {labels.count(0)} negative)"
        )
    ...
    def _add_negative_samples(
        ...
        negative_candidates = [
            item for item in available_candidates if item not in positive_items
        ]

...
class NCFTrainer:
    ...
    def compute_validation_loss(self, test_ratings, total_users_to_test=None):
        ...
        try:
            for (u, i) in tqdm(test_user_item_set[:total_users_to_test], desc="Computing val loss", leave=False):
                ...
                movie_feat = self.model.feature_processor.get_movie_features(i).unsqueeze(0).to(self.device)
                
                with torch.no_grad():
                    loss = self.model.compute_loss((user_feat, movie_feat, torch.ones(1).to(self.device)))
                    total_loss += loss.item()
                    num_cases += 1
    ...
    def validate_model(self, test_ratings, precomputed_candidates, total_users_to_test=None, k=10):
        self.model.eval()
        with torch.no_grad():
    ...
    def train(
        ...
        for epoch in tqdm(range(num_epochs), desc="Training"):
            ...
            hit_ratio, mrr, mean_rank = self.validate_model(
                self.validation_ratings, self.validation_candidates,
                total_users_to_test=total_users_to_test, k=k
            )
    ...
    def plot_metrics(self, save_path: Optional[str] = None):
        ...
        axes[0, 1].plot(epochs, self.val_metrics['hit_ratio'], 'g-', label='Hit Ratio @ 10', linewidth=2)
